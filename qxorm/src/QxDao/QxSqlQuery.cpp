...
QHash<QString, int> QxSqlQuery::getSqlResultAllColumns() const
{
   if (! m_pSqlResult) { return QHash<QString, int>(); }
...
void QxSqlQuery::dumpSqlResult()
{
   ...
   QVector<QString> lstColumns(m_pSqlResult->positionByKey.count());
   QHashIterator<QString, int> itr(m_pSqlResult->positionByKey);
   while (itr.hasNext()) { itr.next(); lstColumns[itr.value()] = itr.key(); }
...
{
   QSqlDatabase d = (pDatabase ? (* pDatabase) : qx::QxSqlDatabase::getDatabase());
