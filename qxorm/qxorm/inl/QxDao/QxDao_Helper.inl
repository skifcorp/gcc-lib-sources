...
#include <qxorm/QxDao/IxDao_Helper.h>
...
public:
    ...
    QxDao_Helper(T & t, QSqlDatabase * pDatabase, const QString & sContext, const QString& tn=QString()) : IxDao_Helper(), table_name(tn)
    {
        Q_UNUSED(t);
...
protected:
   ...
   {
        m_pQueryBuilder.reset(new qx::QxSqlQueryBuilder<T>());
        //qDebug() << "QxDao_Helper::updateQueryBuilder! table_name: " <<  table_name;

        if ( !table_name.isEmpty() ) {
            //qDebug() << "   table_name NOT EMPTY so setting table name!";
            m_pQueryBuilder->setTableName(table_name);
        }
   }
private:
    QString table_name;
