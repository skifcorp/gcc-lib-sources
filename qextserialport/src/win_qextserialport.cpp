...
bool QextSerialPort::open(OpenMode mode) {
        ...
        //qDebug() << "URAAAAAAAAAAAAAAAAAAAAA!";
    ...
    if (!isOpen()) {
        ...
        if (Win_Handle!=INVALID_HANDLE_VALUE) {
            ...
            if (queryMode() == QextSerialPort::EventDriven) {
                ...
                //qDebug () << "555550---" <<  reinterpret_cast<uint> (overlap.hEvent);
...
void QextSerialPort::flush() {
    ...
    if (isOpen()) {
        ...
        PurgeComm(Win_Handle, PURGE_RXABORT | PURGE_RXCLEAR | PURGE_TXABORT | PURGE_TXCLEAR);
    }
}
